...
class ChronoEditRotaryPosEmbed(nn.Module):
    def __init__(
        ...
        for dim in [t_dim, h_dim, w_dim]:
            freq = get_1d_rotary_pos_embed(
                dim, max_seq_len, theta, use_real=False, repeat_interleave_real=False, freqs_dtype=torch.float32  # Changed to float32
            )
            # Handle case where get_1d_rotary_pos_embed returns a tuple
            if isinstance(freq, tuple):
                freq = freq[0]  # Take the first element
            freqs.append(freq)
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        freqs = list(self.freqs.split_with_sizes(
            [
        ...
        # Ensure we're working with tensors, not tuples
        for i in range(len(freqs)):
            if isinstance(freqs[i], tuple):
                freqs[i] = freqs[i][0]
        
        # Ensure sufficient length for temporal frequencies
        if freqs[0].size(0) < self.temporal_skip_len:
            # Extend by repeating the last element
            repeat_count = self.temporal_skip_len - freqs[0].size(0)
            extended_part = freqs[0][-1:].repeat(repeat_count, 1)
            freqs_0 = torch.cat([freqs[0], extended_part], dim=0)
        else:
            freqs_0 = freqs[0]

        if num_frames == 2:
            # Safely index the first and last elements
            first_elem = freqs_0[0:1]   # Shape: [1, dim]
            last_elem = freqs_0[-1:]    # Shape: [1, dim]
            selected_freqs = torch.cat([first_elem, last_elem], dim=0)  # Shape: [2, dim]
            freqs_f = selected_freqs.view(ppf, 1, 1, -1).expand(ppf, pph, ppw, -1)
        else:
            freqs_f = freqs_0[:ppf].view(ppf, 1, 1, -1).expand(ppf, pph, ppw, -1)
